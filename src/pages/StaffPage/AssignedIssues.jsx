...
import AssignedIssuesSkeleton from '../../Components/AssignedIssuesSkeleton';
...
const AssignedIssues = () => {
    ...
    if (issuesLoading) return (
        <AssignedIssuesSkeleton></AssignedIssuesSkeleton>
    );
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
                        <div className="flex items-center gap-4">
                            <div className="relative shrink-0">
                                <div className="w-14 h-14 bg-linear-to-br from-blue-600 to-cyan-500 rounded-xl flex items-center justify-center shadow-lg shadow-blue-200 dark:shadow-blue-900/40">
                            ...
                            <div>
                                <h1 className="text-3xl sm:text-4xl md:text-4xl font-bold text-gray-900 dark:text-white">
                ...
                <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700 p-5 mb-6 transition-colors">
                    ...
                    {(searchTerm || statusFilter !== 'all' || priorityFilter !== 'all' || categoryFilter !== 'all') && (
                        <div className="mt-5 pt-4 border-t border-gray-100 dark:border-gray-700 flex flex-wrap items-center gap-2">
                            ...
                            {searchTerm && (
                                <span className="inline-flex items-center gap-1.5 px-3 py-1 bg-blue-50 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300 text-xs font-semibold rounded-full border border-blue-100 dark:border-blue-800">
                                    <FaSearch className="w-3 h-3" />"{searchTerm}"
                                    <button onClick={() => { setSearchTerm(''); setCurrentPage(1); }} className="ml-0.5 hover:opacity-70 transition-opacity"><FaTimes className="w-3 h-3" /></button>
                                </span>
                            )}
                            {statusFilter !== 'all' && (
                                <span className="inline-flex items-center gap-1.5 px-3 py-1 bg-amber-50 dark:bg-amber-900/30 text-amber-700 dark:text-amber-300 text-xs font-semibold rounded-full border border-amber-100 dark:border-amber-800">
                                    <FaClock className="w-3 h-3" />{STATUS_CONFIG[statusFilter]?.label}
                                    <button onClick={() => { setStatusFilter('all'); setCurrentPage(1); }} className="ml-0.5 hover:opacity-70 transition-opacity"><FaTimes className="w-3 h-3" /></button>
                                </span>
                            )}
                            {priorityFilter !== 'all' && (
                                <span className="inline-flex items-center gap-1.5 px-3 py-1 bg-red-50 dark:bg-red-900/30 text-red-700 dark:text-red-300 text-xs font-semibold rounded-full border border-red-100 dark:border-red-800">
                                    <FaExclamationTriangle className="w-3 h-3" />{PRIORITY_CONFIG[priorityFilter]?.label}
                                    <button onClick={() => { setPriorityFilter('all'); setCurrentPage(1); }} className="ml-0.5 hover:opacity-70 transition-opacity"><FaTimes className="w-3 h-3" /></button>
                                </span>
                            )}
                            {categoryFilter !== 'all' && (
                                <span className="inline-flex items-center gap-1.5 px-3 py-1 bg-green-50 dark:bg-green-900/30 text-green-700 dark:text-green-300 text-xs font-semibold rounded-full border border-green-100 dark:border-green-800">
                                    <FaTag className="w-3 h-3" />{categoryFilter}
                                    <button onClick={() => { setCategoryFilter('all'); setCurrentPage(1); }} className="ml-0.5 hover:opacity-70 transition-opacity"><FaTimes className="w-3 h-3" /></button>
                                </span>
                            )}
                            <button onClick={() => { setSearchTerm(''); setStatusFilter('all'); setPriorityFilter('all'); setCategoryFilter('all'); setCurrentPage(1); }}
                                                        ...
                                                        <div className="flex items-start gap-3">
                                                            ...
                                                            <div className="min-w-0">
                                                                <div className="flex items-center gap-2 mb-1 flex-wrap">
                                                                    ...
                                                                    {issue.isBoosted && (
                                                                        <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md text-xs font-bold bg-linear-to-r from-purple-500 to-pink-500 text-white flex-shrink-0">
                                                                            <FaArrowUp className="w-2.5 h-2.5" />Boosted
                                                                        </span>
                                                                    )}
                                                                </div>
                                                                <div className="flex items-center gap-1.5 text-xs text-gray-400 dark:text-gray-500">
                                                                    <span className="flex items-center gap-1"><FaCalendarAlt className="w-2.5 h-2.5" />Created: {formatDate(issue.createdAt)}</span>
                                                                    <span className="flex items-center gap-1"><FaCalendarDay className="w-2.5 h-2.5" />Updated: {formatDate(issue.updatedAt)}</span>
                                                                </div>
                                                            </div>
                                                        </div>
                                                    </td>
                                                    ...
                                                    <td className="py-4 px-5">
                                                        <div className="flex items-center gap-2">
                                                            <button onClick={() => navigate(`/issueDetailsPage/${issue._id}`)}
                                                                className="flex items-center gap-1.5 px-3 py-2 bg-blue-50 dark:bg-blue-900/30 border border-blue-200 dark:border-blue-700 text-blue-700 dark:text-blue-300 text-xs font-semibold rounded-xl hover:bg-blue-100 dark:hover:bg-blue-900/50 transition-all">
                                                                <FaEye className="w-3.5 h-3.5" />Details
                                                            </button>

                                                            <div className="relative">
                                                                <button onClick={() => setStatusDropdownOpen(isOpen ? null : issue._id)}
                                ...
                                return (
                                    <motion.div key={issue._id}
                                        ...
                                        <div className="p-4">
                                            ...
                                            <div className="flex gap-2">
                                                <button onClick={() => navigate(`/issueDetailsPage/${issue._id}`)}
                                                    className="flex-1 flex items-center justify-center gap-1.5 py-2.5 bg-blue-50 dark:bg-blue-900/30 border border-blue-200 dark:border-blue-700 text-blue-700 dark:text-blue-300 text-xs font-semibold rounded-xl hover:bg-blue-100 dark:hover:bg-blue-900/50 transition-all">
